...
function Next
  ...
  algorithm
    ...
    y := Prob(k, i, j);   // Calcolo la probabilità di far andare il team k dalla fase i alla fase 1
    ...
    while ((p > y) and (j < Prm.N)) loop
      ...
      y := y + Prob(k, i, j);     // Calcolo la probabilità di andare da fase i a fase j
