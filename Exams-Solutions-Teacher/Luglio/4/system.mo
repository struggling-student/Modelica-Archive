...
OutputReal ypy[Prm.N, Prm.Q];   //  Anomaly Detection Signal, parallele computation

Terminator term;

Monitor monitor[Prm.N, Prm.Q];
...
for i in 1:Prm.N loop
	...
	ypy[i, j] = AnomalyDetection(c1[i,j].ValueFromServer, c1[i,j].Mavg.y) ;	
        c1[i,j].ValueFromServer = monitor[i, j].ValueFromServer ;
        c1[i,j].Mavg.y = monitor[i, j].ValueMobAvg ;
        monitor[i, j].AnomalySignal = AnomalyDetection(c1[i,j].ValueFromServer, c1[i,j].Mavg.y) ;
   end for;
end for;
